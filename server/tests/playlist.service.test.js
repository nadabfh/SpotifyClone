...
describe("Playlist Service", () => {
  ...
  beforeEach(async () => {
    ...
    await dbService.db
      ...
      .insertMany([playlists[0], playlists[1]]);
  ...
  it("addPlaylist should add a playlist", async () => {
    jest
      .spyOn(playlistService, "chooseProperEncoding")
      .mockImplementation(() => {
        return Promise.resolve("jpeg");
      });
    jest
      .spyOn(playlistService, "savePlaylistThumbnail")
      .mockImplementation(() => {
  ...
  it("updatePlaylist should update a playlist", async () => {
    ...
    const modifiedPlaylist = await playlistService.getPlaylistById(
      updatedPlaylist.id
    );
  ...
  it("deletePlaylist should delete a playlist", async () => {
    const spy = jest
      .spyOn(playlistService, "deletePlaylistThumbnail")
      .mockImplementation(() => {
        return Promise.resolve();
      });
  ...
  it("chooseProperEncoding should choose the proper encoding (jpeg)", async () => {
    const encoding = await playlistService.chooseProperEncoding(
  ...
  it("savePlaylistThumbnail should save the thumbnail", async () => {
    ...
    expect(fsSpy).toBeCalledWith(filePath, thumbnailData, {
      encoding: "base64",
    });
  });
  ...
  it("should call populateDb() of DatabaseService", async () => {
    const spy = jest
      .spyOn(playlistService.dbService, "populateDb")
      .mockImplementation(() => {});
